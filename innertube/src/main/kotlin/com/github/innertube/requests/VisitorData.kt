...
private val visitorDataRegex = Regex(""""VISITOR_DATA"\s*:\s*"([^"]+)"""", RegexOption.IGNORE_CASE)

suspend fun Innertube.visitorData(): Result<String> = runCatching {
    Innertube.visitorData?.let { return@runCatching it }
    ...
    if (token != null && Regex("^Cg([ts])").containsMatchIn(token)) {
        token.also { Innertube.visitorData = it }
    } else {
        error("Failed to scrape VISITOR_DATA from YouTube Music HTML.")
    }
}